...
<script>
  ...
  export default{
    ...
    mounted() {
      ...
      this.$eventBus.$on("deviceListDataReturn",this.deviceListDataReturn);
    },
    methods:{
      //根据返回的结果绘制分页,这里的status是父组件传过来的组件标志，无论父组件是否传值都可以赋值
      deviceListDataReturn:function (result,status){
        this.componentType = status;   //定义组件类型
        let pageData =[];
      ...
      getPageData:function(index){
         this.$eventBus.$emit('updataPage',index,this.componentType);  //将点击后的分页码传给父级并根据分页码获取新的数据
